...
const offset = computed(() =>
  mode.value === 'days' ? new Date(curr.year, curr.month - 1, 1).getDay() : 0
)
...
function isToday(/** 1-indexed */ i: number): boolean {
  if (mode.value === 'days') {
    return now.day === i && curr.month === now.month && curr.year === now.year
  }
  return false
  // if (mode.value === 'months') {
...
<template>
  <div class="SDatePickerBase" :class="mode" @keydown="onKeydown">
    <div class="header">
      ...
      <button type="button" @click="onZoomOut" :disabled="mode === 'years'" class="desc">
        {{ description }}
      </button>
