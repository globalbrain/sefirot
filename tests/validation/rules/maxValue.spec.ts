...
describe('validation/rules/maxValue', () => {
  ...
  test('it validates whether the value is valid date', () => {
    const maxDate = 'Fri Jan 19 2024 14:55:27 GMT+0900 (Japan Standard Time)'
    const invalidDate = 'Fri Jan 19 2024 14:55:28 GMT+0900 (Japan Standard Time)'
    const rule = maxValue(+(new Date(maxDate)))

    expect(rule.$validator(undefined, null, null)).toBe(true)
    expect(rule.$validator(null, null, null)).toBe(true)
    expect(rule.$validator(new Date(maxDate), null, null)).toBe(true)
